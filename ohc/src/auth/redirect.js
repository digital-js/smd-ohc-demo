...
myMSALObj
  ...
  .then((response) => {
    if (response) {
       ...
       */
      if (
        response.idTokenClaims['tfp'].toUpperCase() === b2cPolicies.names.signUpSignIn.toUpperCase()
      ) {
...
/**
 * @param {msal.AccountInfo | null} account
 */
...
/**
 * @param {msal.RedirectRequest | msal.SilentRequest} request
 */
